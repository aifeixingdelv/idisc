...
class AutoMineDataset(BaseDataset):
    CAM_INTRINSIC = {
        "AutoMineDepth": torch.tensor(
            [
    ...
    min_depth = 0.01
    max_depth = 80
    test_split = "automine_depth_test.txt"
    train_split = "automine_depth_train.txt"
    ...
    def load_dataset(self):
        ...
        with open(os.path.join(self.base_path, "AutoMine-Depth", self.split_file)) as f:
            for line in f:
                img_info = dict()
    ...
    def preprocess_crop(self, image, gts=None, info=None):
        height_start, width_start = (int((image.shape[0] - self.height) / 2),
                                     int((image.shape[1] - self.width) / 2))
